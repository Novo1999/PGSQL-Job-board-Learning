...
import { handleDbError, pagination, paginated, queryBool } from '../utils/http.js';
...
export async function listSavedJobs(req: Request<UserParams>, res: Response): Promise<void> {
  ...
  try {
    const result: QueryResult<SavedJobListItemRow> = await pool.query<SavedJobListItemRow>(``, [
      req.params.userId,
      queryBool(req.query.actionable_only) ?? false,
      page.limit,
      page.offset,
    ]);
...
export async function saveJob(
  req: Request<UserParams, unknown, SaveJobInput>,
  res: Response
): Promise<void> {
  if (!req.body?.job_id) {
